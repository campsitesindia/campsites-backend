...
import com.dd.campsites.service.FeaturesQueryService;
import com.dd.campsites.service.FeaturesService;
import com.dd.campsites.service.campsitesindia.FeatureBusinessService;
...
/**
 * REST controller for managing {@link com.dd.campsites.domain.Features}.
 */
@RestController
@RequestMapping("/api")
public class FeaturesResource {
    ...
    /**
     * {@code GET  /features/listing/:id} : get the "id" features.
     *
     * @param id the criteria which the requested entities should match.
     * @return the {@link ResponseEntity} with status {@code 200 (OK)} and the list of features in body.
     */
    @GetMapping("/features/listing/{id}")
    public ResponseEntity<List<Features>> getAllFeaturesByListingId(@PathVariable Long id) {
        log.debug("REST request to get Features by ListingId: {}", id);
