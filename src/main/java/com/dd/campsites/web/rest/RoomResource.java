...
import com.dd.campsites.service.RoomQueryService;
import com.dd.campsites.service.RoomService;
import com.dd.campsites.service.campsitesindia.RoomBusinessService;
...
/**
 * REST controller for managing {@link com.dd.campsites.domain.Room}.
 */
@RestController
@RequestMapping("/api")
public class RoomResource {
    ...
    /**
     * {@code GET  /rooms/listing/:id} : get the "id" room.
     *

     * @param id the criteria which the requested entities should match.
     * @return the {@link ResponseEntity} with status {@code 200 (OK)} and the list of rooms in body.
     */
    @GetMapping("/rooms/listing/{id}")
    public ResponseEntity<List<Room>> getAllRoomsByListingId(@PathVariable long id) {
        log.debug("REST request to get Rooms by ListingId: {}", id);
        List<Room> roomList = roomBusinessService.findByListingId(id);
        // HttpHeaders headers = PaginationUtil.generatePaginationHttpHeaders(ServletUriComponentsBuilder.fromCurrentRequest(), page);
