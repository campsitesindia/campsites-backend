...
import com.dd.campsites.repository.TagRepository;
import com.dd.campsites.service.criteria.TagCriteria;
...
class TagResourceIT {
    ...
    @Test
    @Transactional
    void getAllTagsByPhotosIsEqualToSomething() throws Exception {
        // Initialize the database
        tagRepository.saveAndFlush(tag);
        Photos photos = PhotosResourceIT.createEntity(em);
        em.persist(photos);
        em.flush();
        tag.addPhotos(photos);
        tagRepository.saveAndFlush(tag);
        Long photosId = photos.getId();

        // Get all the tagList where photos equals to photosId
        defaultTagShouldBeFound("photosId.equals=" + photosId);

        // Get all the tagList where photos equals to (photosId + 1)
        defaultTagShouldNotBeFound("photosId.equals=" + (photosId + 1));
    }
