...
class ListingTypeResourceIT {
    ...
    void getAllListingTypesByParentIsEqualToSomething() throws Exception {
        // Initialize the database
        listingTypeRepository.saveAndFlush(listingType);
        ListingType parent = ListingTypeResourceIT.createEntity(em);
        em.persist(parent);
        em.flush();
        listingType.setParent(parent);
        listingTypeRepository.saveAndFlush(listingType);
        Long parentId = parent.getId();

        // Get all the listingTypeList where parent equals to parentId
        defaultListingTypeShouldBeFound("parentId.equals=" + parentId);

        // Get all the listingTypeList where parent equals to (parentId + 1)
        defaultListingTypeShouldNotBeFound("parentId.equals=" + (parentId + 1));
    }
    ...
    void partialUpdateListingTypeWithPatch() throws Exception {
        ...
        partialUpdatedListingType.title(UPDATED_TITLE).thumbnail(UPDATED_THUMBNAIL).color(UPDATED_COLOR).updateDate(UPDATED_UPDATE_DATE);
