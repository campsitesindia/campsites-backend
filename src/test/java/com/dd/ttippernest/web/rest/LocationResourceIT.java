...
class LocationResourceIT {
    ...
    void createLocation() throws Exception {
        ...
        assertThat(testLocation.getParent()).isEqualTo(DEFAULT_PARENT);
    ...
    void getAllLocationsByParentIsEqualToSomething() throws Exception {
        ...
        // Get all the locationList where parent equals to DEFAULT_PARENT
        defaultLocationShouldBeFound("parent.equals=" + DEFAULT_PARENT);

        // Get all the locationList where parent equals to UPDATED_PARENT
        defaultLocationShouldNotBeFound("parent.equals=" + UPDATED_PARENT);
    }

    @Test
    @Transactional
    void getAllLocationsByParentIsNotEqualToSomething() throws Exception {
        // Initialize the database
        locationRepository.saveAndFlush(location);

        // Get all the locationList where parent not equals to DEFAULT_PARENT
        defaultLocationShouldNotBeFound("parent.notEquals=" + DEFAULT_PARENT);

        // Get all the locationList where parent not equals to UPDATED_PARENT
        defaultLocationShouldBeFound("parent.notEquals=" + UPDATED_PARENT);
    }

    @Test
    @Transactional
    void getAllLocationsByParentIsInShouldWork() throws Exception {
        // Initialize the database
        locationRepository.saveAndFlush(location);

        // Get all the locationList where parent in DEFAULT_PARENT or UPDATED_PARENT
        defaultLocationShouldBeFound("parent.in=" + DEFAULT_PARENT + "," + UPDATED_PARENT);

        // Get all the locationList where parent equals to UPDATED_PARENT
        defaultLocationShouldNotBeFound("parent.in=" + UPDATED_PARENT);
    }

    @Test
    @Transactional
    void getAllLocationsByParentIsNullOrNotNull() throws Exception {
        // Initialize the database
        locationRepository.saveAndFlush(location);

        // Get all the locationList where parent is not null
        defaultLocationShouldBeFound("parent.specified=true");

        // Get all the locationList where parent is null
        defaultLocationShouldNotBeFound("parent.specified=false");
    }

    @Test
    @Transactional
    void getAllLocationsByParentIsGreaterThanOrEqualToSomething() throws Exception {
        // Initialize the database
        locationRepository.saveAndFlush(location);

        // Get all the locationList where parent is greater than or equal to DEFAULT_PARENT
        defaultLocationShouldBeFound("parent.greaterThanOrEqual=" + DEFAULT_PARENT);

        // Get all the locationList where parent is greater than or equal to UPDATED_PARENT
        defaultLocationShouldNotBeFound("parent.greaterThanOrEqual=" + UPDATED_PARENT);
    }

    @Test
    @Transactional
    void getAllLocationsByParentIsLessThanOrEqualToSomething() throws Exception {
        // Initialize the database
        locationRepository.saveAndFlush(location);

        // Get all the locationList where parent is less than or equal to DEFAULT_PARENT
        defaultLocationShouldBeFound("parent.lessThanOrEqual=" + DEFAULT_PARENT);

        // Get all the locationList where parent is less than or equal to SMALLER_PARENT
        defaultLocationShouldNotBeFound("parent.lessThanOrEqual=" + SMALLER_PARENT);
    }

    @Test
    @Transactional
    void getAllLocationsByParentIsLessThanSomething() throws Exception {
        // Initialize the database
        locationRepository.saveAndFlush(location);

        // Get all the locationList where parent is less than DEFAULT_PARENT
        defaultLocationShouldNotBeFound("parent.lessThan=" + DEFAULT_PARENT);

        // Get all the locationList where parent is less than UPDATED_PARENT
        defaultLocationShouldBeFound("parent.lessThan=" + UPDATED_PARENT);
    }

    @Test
    @Transactional
    void getAllLocationsByParentIsGreaterThanSomething() throws Exception {
        // Initialize the database
        locationRepository.saveAndFlush(location);

        // Get all the locationList where parent is greater than DEFAULT_PARENT
        defaultLocationShouldNotBeFound("parent.greaterThan=" + DEFAULT_PARENT);

        // Get all the locationList where parent is greater than SMALLER_PARENT
        defaultLocationShouldBeFound("parent.greaterThan=" + SMALLER_PARENT);
    }
    ...
    @Test
    @Transactional
    void getAllLocationsByParentLocationIsEqualToSomething() throws Exception {
        // Initialize the database
        locationRepository.saveAndFlush(location);
        Location parentLocation = LocationResourceIT.createEntity(em);
        em.persist(parentLocation);
        em.flush();
        location.setParentLocation(parentLocation);
        locationRepository.saveAndFlush(location);
        Long parentLocationId = parentLocation.getId();

        // Get all the locationList where parentLocation equals to parentLocationId
        defaultLocationShouldBeFound("parentLocationId.equals=" + parentLocationId);

        // Get all the locationList where parentLocation equals to (parentLocationId + 1)
        defaultLocationShouldNotBeFound("parentLocationId.equals=" + (parentLocationId + 1));
    }
    ...
    void putNewLocation() throws Exception {
        ...
        assertThat(testLocation.getParent()).isEqualTo(UPDATED_PARENT);
